...
const TILE_IMAGE_PATH := "res://demo/assets/tiles/ocean_rock_modular_tiles.png"
const TILESET_PATH := "res://demo/assets/tiles/ocean_rock_modular_tileset.tres"
const WORKSPACE_SCENE_PATH := "res://demo/scenes/tools/tile_paint_workspace.tscn"
...
func _create_workspace_scene(tile_set: TileSet) -> PackedScene:
	...
	root.set_script(load("res://demo/scripts/tile_workspace.gd"))

	var atmosphere := Node2D.new()
	atmosphere.name = "Atmosphere"
	atmosphere.set_script(load("res://demo/scripts/demo_atmosphere.gd"))
	...
	canvas.name = "DemoCanvasModulate"
	...
	var player_scene := load("res://demo/scenes/player.tscn") as PackedScene
	...
	hud.name = "HUD"
	hud.set_script(load("res://demo/scripts/health_ui.gd"))
	_add_owned(root, hud)

	var exit_menu := CanvasLayer.new()
	exit_menu.name = "ExitMenu"
	exit_menu.process_mode = Node.PROCESS_MODE_ALWAYS
	exit_menu.set_script(load("res://demo/scripts/demo_exit_menu.gd"))
	_add_owned(root, exit_menu)

	var game_ui_scene := load("res://demo/scenes/game_ui.tscn") as PackedScene
