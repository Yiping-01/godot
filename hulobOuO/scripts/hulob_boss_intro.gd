...
func _on_body_entered(body: Node2D) -> void:
	...
	if scene != null and scene.has_method("_on_hulob_boss_intro_started"):
		scene.call("_on_hulob_boss_intro_started", boss)

	HULOB_COMBAT_JUICE.shake_camera(self, 0.55, 8.5)
	queue_free()
